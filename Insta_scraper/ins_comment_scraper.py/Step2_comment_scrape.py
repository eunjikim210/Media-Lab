...
class Com(object):
    ...
    def     __init__(self):
        df = pandas.read_excel("post.xlsx",dtype=str)
    ...
    def _download(self,item):
        ...
        api = f'https://www.instagram.com/api/v1/media/{media_id}/comments/'
        ...
        for page in range(media_count_page):
            ...
            print(res)
            try:
                edges = res.get("comments",[])
            except Exception as e:
                print(f"parse  edges error:{e}")
                edges = []
            for comment in edges:
                try:
                    # Post details page: replier's nickname, replier's profile link, replier's verification status, number of likes for the reply, reply time, reply count, and ideally, associate the reply and the relationship between the reply and the sub-replies.
                    #
                    saveitem = {}
                    ...
                    saveitem["comment_reply_count"] = comment.get("child_comment_count")
            ...
            if res.get("has_more_headload_comments") is False:
                break

            try:
                next_min_id = res['next_min_id']
    ...
    def parse(self):
        for rec in self.records[14000:16000]:
            self.start_count += 1
            if type(rec['post_id']) != str:
                print(rec['post_id'])
                continue
            print(f">>> Visiting: {rec['post_id'] }  {self.start_count} ")
            try:
                self._download(rec)
            except Exception as e:
                print(f">>> error: {e}")
...
if __name__ == "__main__":
    ...
    with open(f"temp{db_number}.txt", 'r', encoding='utf-8') as f:
        lines = [json.loads(i.strip()) for i in f.readlines()]
        lines_sort = sorted(lines,key=lambda x:x.get("comment_reply_count"),reverse=True)
        filter_lines = [i for i in lines_sort if i.get("comment_reply_count") > 0]
        df = pandas.DataFrame(lines_sort)
        df.drop_duplicates(['post_id', 'comment_id'], inplace=True)
        df.to_csv(f"comments{db_number}.csv",index=False)
